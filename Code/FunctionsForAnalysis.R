...
getCleanSIAs <- function(codes, dataInd, dataSIA, input_country) {
	...
	dataSIA_country$Regions[dataSIA_country$Extent=="National"] <- 0
	

	# If there isn't any information on Regions, assume National campaign and set Regions equal to 0
	dataSIA_country$Regions[is.na(dataSIA_country$Regions)] <- 0

	
	# Format the date
	dataSIA_country$Start.date <- as.Date(dataSIA_country$Start.date, format="%m/%d/%Y")
	dataSIA_country$End.date <- as.Date(dataSIA_country$End.date, format="%m/%d/%Y")
  
	# Translate the Age.Group column to useable age range of each SIA campaign
	dataSIA_country$age_ub_mos <- NA
	dataSIA_country$age_lb_mos <- NA
...
dataSIA_country<-dataSIA_country%>%
  ...
  dplyr::filter(Age.group != "unknown")
		...
		if(dataSIA_country$Age.group[i] == "18-23 M"){
		  dataSIA_country$age_lb_mos[i] <- 18
		  dataSIA_country$age_ub_mos[i] <- 23}

		if(dataSIA_country$Age.group[i] == "9 M-4 Y"){
		  dataSIA_country$age_lb_mos[i] <- 9
		  dataSIA_country$age_ub_mos[i] <- 59}	
		  
		if(dataSIA_country$Age.group[i]=="0-11 M"){
			dataSIA_country$age_lb_mos[i] <- 0
			dataSIA_country$age_ub_mos[i] <- 11} 	

		if(dataSIA_country$Age.group[i]=="0-59 M"){
			dataSIA_country$age_lb_mos[i] <- 0
			dataSIA_country$age_ub_mos[i] <- 59} 		  
		  
		if(dataSIA_country$Age.group[i]=="0 M-14 Y"){
			dataSIA_country$age_lb_mos[i] <- 0
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1	
			
		if(dataSIA_country$Age.group[i]=="6-59 M"){
			dataSIA_country$age_lb_mos[i] <- 6
			dataSIA_country$age_ub_mos[i] <- 59}
		
		if(dataSIA_country$Age.group[i]=="6 M-9 Y"){ 
			dataSIA_country$age_lb_mos[i] <- 6
			dataSIA_country$age_ub_mos[i] <- 119} # 12*10-1	

		if(dataSIA_country$Age.group[i]=="6 M-10 Y"){ 
			dataSIA_country$age_lb_mos[i] <- 6
			dataSIA_country$age_ub_mos[i] <- 131} # 12*11-1	
		
		if(dataSIA_country$Age.group[i]=="6 M-14 Y"){
			dataSIA_country$age_lb_mos[i] <- 6
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1
		
		if(dataSIA_country$Age.group[i]=="6 M-15 Y"){
			dataSIA_country$age_lb_mos[i] <- 6
			dataSIA_country$age_ub_mos[i] <- 191} # 12*16-1
		  
		if(dataSIA_country$Age.group[i]=="9 M-24 Y"){
		  dataSIA_country$age_lb_mos[i] <- 9
		  dataSIA_country$age_ub_mos[i] <- 299} #12*25-1
		  
		if(dataSIA_country$Age.group[i]=="9-23 M"){
			dataSIA_country$age_lb_mos[i] <- 9
			dataSIA_country$age_ub_mos[i] <- 23} 
	
		if(dataSIA_country$Age.group[i]=="9-47 M"){
			dataSIA_country$age_lb_mos[i] <- 9
			dataSIA_country$age_ub_mos[i] <- 47} 
		
		if(dataSIA_country$Age.group[i]=="9-49 M"){
			dataSIA_country$age_lb_mos[i] <- 9
			dataSIA_country$age_ub_mos[i] <- 49} 
	  
		if(dataSIA_country$Age.group[i]=="9-59 M"){
			dataSIA_country$age_lb_mos[i] <- 9
			dataSIA_country$age_ub_mos[i] <- 59}
		
		if(dataSIA_country$Age.group[i]=="9-36 M"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 36} 
	  
		if(dataSIA_country$Age.group[i]=="9 M-10 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 131} # 12*11-1		
		  
		if(dataSIA_country$Age.group[i]=="9 M-12 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 155} # 12*13-1		
		
		if(dataSIA_country$Age.group[i]=="9 M-14 Y"){
			dataSIA_country$age_lb_mos[i] <- 9
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1
		
		if(dataSIA_country$Age.group[i]=="9 M-15 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 191} # 12*16-1
		
		if(dataSIA_country$Age.group[i] == "9 M+"){
		  dataSIA_country$age_lb_mos[i] <- 9
		  dataSIA_country$age_ub_mos[i] <- 179
		} #Assuming upper limit is 14

    if(dataSIA_country$Age.group[i] == "6 M+"){
		  dataSIA_country$age_lb_mos[i] <- 6
		  dataSIA_country$age_ub_mos[i] <- 179
		} #Assuming upper limit is 14
		  
		if(dataSIA_country$Age.group[i] == "0-10 Y"){
		  dataSIA_country$age_lb_mos[i] <- 0
		  dataSIA_country$age_ub_mos[i] <- 119
		}#12*10-1
		  
		if(dataSIA_country$Age.group[i]=="<15 Y"){ 
			dataSIA_country$age_lb_mos[i] <- 9	# Assuming starts at 9 months
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1
		  
		if(dataSIA_country$Age.group[i]=="<1 Y"){ 
		    dataSIA_country$age_lb_mos[i] <- 0	# Assuming starts at 0 months
		    dataSIA_country$age_ub_mos[i] <- 11} # 12*1-1
		  
		if(dataSIA_country$Age.group[i] == "6 Y"){
		  dataSIA_country$age_lb_mos[i] <- 72
		  dataSIA_country$age_ub_mos[i] <- 83} #Assuming 6 year olds

		if(dataSIA_country$Age.group[i]=="18-23 M"){
			dataSIA_country$age_lb_mos[i] <- 18
			dataSIA_country$age_ub_mos[i] <- 23} 
		
		if(dataSIA_country$Age.group[i]=="0-23 M"){
		    dataSIA_country$age_lb_mos[i] <- 0
		    dataSIA_country$age_ub_mos[i] <- 23} 
		
		if(dataSIA_country$Age.group[i]=="6-47 M"){
		    dataSIA_country$age_lb_mos[i] <- 6
		    dataSIA_country$age_ub_mos[i] <- 47} 
		
		if(dataSIA_country$Age.group[i]=="12-59 M"){
		    dataSIA_country$age_lb_mos[i] <- 12
		    dataSIA_country$age_ub_mos[i] <- 59}
		
		if(dataSIA_country$Age.group[i]=="15-23 M"){
		    dataSIA_country$age_lb_mos[i] <- 15
		    dataSIA_country$age_ub_mos[i] <- 23}  
		  
		if(dataSIA_country$Age.group[i]=="6-180 M"){
		    dataSIA_country$age_lb_mos[i] <- 6
		    dataSIA_country$age_ub_mos[i] <- 180}  
		  
		if(dataSIA_country$Age.group[i]=="6 M-<15 Y"){# Assuming this means less than 15 years (same as 14 Y)
		  dataSIA_country$age_lb_mos[i] <- 6
		  dataSIA_country$age_ub_mos[i] <- 179}

		if(dataSIA_country$Age.group[i]=="6 M-179 M"){
		  dataSIA_country$age_lb_mos[i] <- 6
		  dataSIA_country$age_ub_mos[i] <- 179}
		  
		if(dataSIA_country$Age.group[i]=="6-14 Y"){ # Assuming the 6 means 6 years
			dataSIA_country$age_lb_mos[i] <- 72	# 12*6
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1	
		  
		if(dataSIA_country$Age.group[i]=="6-10 Y"){ # Assuming the 6 means 6 years
		    dataSIA_country$age_lb_mos[i] <- 72	# 12*6
		    dataSIA_country$age_ub_mos[i] <- 131} # 12*11-1		  

		if(dataSIA_country$Age.group[i]=="7-14 Y"){ # Assuming the 7 means 7 years
			dataSIA_country$age_lb_mos[i] <- 84	# 12*7
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1
		  
		if(dataSIA_country$Age.group[i]=="9-15 Y"){ # Assuming the 7 means 7 years
		    dataSIA_country$age_lb_mos[i] <- 108	# 12*9
		    dataSIA_country$age_ub_mos[i] <- 191} # 12*16-1
		  
		if(dataSIA_country$Age.group[i]=="5-14 Y"){ # Assuming the 5 means 5 years
		    dataSIA_country$age_lb_mos[i] <- 60	# 12*5
		    dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1
		  
		if(dataSIA_country$Age.group[i]=="1-5 Y"){ # Assuming the 5 means 5 years
		    dataSIA_country$age_lb_mos[i] <- 12	# 12*5
		    dataSIA_country$age_ub_mos[i] <- 71} # 12*6-1
		  
		if(dataSIA_country$Age.group[i]=="1-4 Y"){ # Assuming the 5 means 5 years
		    dataSIA_country$age_lb_mos[i] <- 12	# 12*5
		    dataSIA_country$age_ub_mos[i] <- 71} # 12*6-1
		  
		if(dataSIA_country$Age.group[i]=="5-12 Y"){ # Assuming the 5 means 5 years
		    dataSIA_country$age_lb_mos[i] <- 60	# 12*5
		    dataSIA_country$age_ub_mos[i] <- 155} # 12*13-1
		  
		if(dataSIA_country$Age.group[i]=="5-29 Y"){ # Assuming the 5 means 5 years
		    dataSIA_country$age_lb_mos[i] <- 60	# 12*5
		    dataSIA_country$age_ub_mos[i] <- 359} # 12*30-1

		if(dataSIA_country$Age.group[i]=="School-age"){ # Assuming this means 6y-14y
			dataSIA_country$age_lb_mos[i] <- 72	# 12*6
			dataSIA_country$age_ub_mos[i] <- 179} # 12*15-1
		
		if(dataSIA_country$Age.group[i]=="9 M-2 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 35} #12*3-1
		
		if(dataSIA_country$Age.group[i]=="9 M-9 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 119} #12*3-1
	  	  
		if(dataSIA_country$Age.group[i]=="6 M-5 Y"){
		    dataSIA_country$age_lb_mos[i] <- 6
		    dataSIA_country$age_ub_mos[i] <- 71} 
		  
	  if(dataSIA_country$Age.group[i]=="9 M-5 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 71} 

		if(dataSIA_country$Age.group[i]=="9 M-39 Y"){
		    dataSIA_country$age_lb_mos[i] <- 9
		    dataSIA_country$age_ub_mos[i] <- 479} # 12*40-1
		
		if(dataSIA_country$Age.group[i]=="6-179 M"){
		  dataSIA_country$age_lb_mos[i] <- 6
		  dataSIA_country$age_ub_mos[i] <- 179}
	...
	# Let's assume that if either the Start.Date or End.Date is unknown, the SIA lasted 7 days
	dataSIA_country$End.date[is.na(dataSIA_country$End.date)] <- dataSIA_country$Start.date[is.na(dataSIA_country$End.date)] + 7
	dataSIA_country$Start.date[is.na(dataSIA_country$Start.date)] <- dataSIA_country$End.date[is.na(dataSIA_country$Start.date)] - 7
	

	# Remove SIAs where both the Start.Date and End.Date are unknown
	dataSIA_country <- dataSIA_country[!is.na(dataSIA_country$Start.date) & !is.na(dataSIA_country$End.date),]
    ...
    earliest_End.date <- min(dataInd$dob_lb)
  
    # Get the latest SIA campaign start date:
    # This is the last survey date
    latest_Start.date <- max(dataInd$survey_date) 
	...
	for(i in 1:nrow(dataSIA_country)) {
			...
			if(as.Date(dataSIA_country$End.date[i], format = "%m/%d/%Y") >= as.Date(earliest_End.date) &
			   as.Date(dataSIA_country$Start.date[i], format = "%m/%d/%Y") <= as.Date(latest_Start.date))
	...
	dataSIA_country$dob_youngest <-NA
	  dataSIA_country$dob_oldest <- NA
 
	for(i in 1:nrow(dataSIA_country)){
	  
	 	  # Youngest eligible individual would turn age_lb_mos on End.Date
	 	  dataSIA_country$dob_youngest[i] <- as.character(min(seq(dataSIA_country$End.date[i], 
   	  length=dataSIA_country$age_lb_mos[i]+1, by="-1 month")))
	 	  
	 	  # Oldest eligible individual would turn age_ub_mos on the day after Start.Date
	 	  dataSIA_country$dob_oldest[i] <-  as.character(min(seq(dataSIA_country$Start.date[i], 
	...
	return(dataSIA_country[,c("Country","Activity","Start.date","End.date","Extent",
