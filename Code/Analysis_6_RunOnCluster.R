...
getGAM_Map_noSubSIA <- function(lower_age, upper_age, i) {
	...
	if(i == 3){
	  ...
	  adm0 <- readShapeSpatial(paste0("./Data/ADM/", codes$ADM_code[i], "_adm/", codes$ADM_code[i], "_adm0_.shp"))
...
# 	adm2@data$DHS_id <- strsplit(sub("_", ".", adm2@data$GID_1), ".", fixed = TRUE) # Order by DHS region name
# 	
# 	
# 	id.rep <- c()
# 	for (i in 1:nrow(adm2@data)){
# 	  
# 	  id.rep[i] <- adm2@data$DHS_id[[i]][2]
# 	  
# 	}
# 	adm2@data <- cbind(adm2@data, id.rep)
# 
# 	adm2@data$id <- rownames(adm2@data)
# 	
# 	adm2@data <- adm2@data%>%
# 	  mutate(DHS_id = id.rep)
# 
# 	# Centroids of province for ID
...
# }
# 
# getResidualVariogram(nugget_km=nugget_km, prop_of_max=prop_of_max, max_dist_km=max_dist_km, interval=interval, nsim=nsim, i=1)
...
for(i in 1:4){
  ...
  getGAM_Map_noSubSIA(lower_age=6, upper_age=24, i)  
  
}
for(i in 1:4){
  
  getPeopleMap_noSubSIA(lower_age=6, upper_age=24, i)  
